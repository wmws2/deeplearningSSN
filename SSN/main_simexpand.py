...
def train(N,subbatchsize):
	...
	if N==1:
		...
		w = np.load('./parameters/simexpand_' + str(N-1) + '_w.npy')
		n = np.load('./parameters/simexpand_' + str(N-1) + '_n.npy')
	...
	for i in tf.range(50):
		...
		for j in range(int(20/multiplier)):
			tf.print(j,end='\r')
			tfn = n
			tfh = construct_h(inputh2[j*5*multiplier:j*5*multiplier+5*multiplier], h)
			tfW = tf.abs(w)*mask
			...
			with tf.GradientTape() as tape:
				...
				ucovbias = tf.square(ucov - targetcov2[j*5*multiplier:j*5*multiplier+5*multiplier])
				uvarbias = tf.linalg.diag_part(ucovbias)

				tempcost = 1e-02 * tf.reduce_mean(tf.square(umean- targetmean2[j*5*multiplier:j*5*multiplier+5*multiplier])) + 2e-02*tf.reduce_mean(uvarbias) + 1e-02 * tf.reduce_mean(tf.square(ucov - targetcov2[j*5*multiplier:j*5*multiplier+5*multiplier]))
